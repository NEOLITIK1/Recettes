...
export default function Historique() {
  ...
  async function fetchAll() {
    ...
    const [{ data: batchData }, { data: rcData }, { data: mpsData }, { data: lignesData }] = await Promise.all([
      supabase.from('batches').select('*').order('date_creation', { ascending: false }),
    ...
    // Attacher les lignes à chaque batch
  ...
  async function handleSeed() {
    for (const batch of BATCHES) {
      ...
      await supabase.from('batches').upsert(batchData, { onConflict: 'id' })
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Tous les batchs produits</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Batchs produits', val: batches.length },
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {batches.map(batch => {
            ...
            const cout = calcCout(lignesEnrichies)
            ...
            return (
              <div key={batch.id} className="bg-white rounded-xl border border-gray-200 p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {batch.date_creation && (
                        <span className="text-xs text-gray-400">{new Date(batch.date_creation).toLocaleDateString('fr-FR')}</span>
                      )}
                      {rc && (
                        <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{rc.nom}</span>
                      )}
                    </div>
                ...
                <div className="flex gap-6 text-sm flex-wrap">
                  <span>
                    <strong className="text-gray-900">{Math.round(masseTotale).toLocaleString('fr-FR')} kg</strong>
                    <span className="text-gray-400 ml-1">total</span>
                  </span>
                  {comp && (
                    <span>
                      <strong className="text-gray-900">{fmt1(comp.pp)}% PP</strong>
                      <span className="text-gray-400 ml-1">/ {fmt1(comp.pe)}% PE / {fmt1(comp.alu)}% Alu</span>
                    </span>
                  )}
                  {cout > 0 && (
                    <span>
                      <strong className="text-gray-900">{Math.round(cout).toLocaleString('fr-FR')} €</strong>
                      <span className="text-gray-400 ml-1">({masseTotale > 0 ? Math.round(cout / masseTotale * 1000) : 0} €/t)</span>
                    </span>
                  )}
                </div>
                {batch.notes && (
                  <p className="mt-2 text-xs text-gray-400 border-t border-gray-50 pt-2">📝 {batch.notes}</p>
                )}
              </div>
            )
          })}
      ...
      <Modal
        ...
        footer={
          <button onClick={() => setDetailBatch(null)} className="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50">
            Fermer
          </button>
        }
      >
