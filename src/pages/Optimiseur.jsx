...
function optimiser(sacsDispo, mpsMap, recette, masseCible, mpsForcees = []) {
  if (!recette) return []
  ...
  function scoreSac(sac, selectionActuelle) {
    ...
    return -(dist + penaliteConcentration)
  }
  ...
  while (totalMasse < masseCible * 0.9 && sacsDisponibles.length > 0 && iterations < 50) {
    ...
    sacsDisponibles.sort((a, b) => scoreSac(b, selection) - scoreSac(a, selection))
    ...
    if (taken > remaining * 1.1) {
      // Utilisation partielle : prendre exactement ce qu'il faut
...
export default function Optimiseur() {
  ...
  function lancer() {
    ...
    const sel = optimiser(sacs, mpsMap, recette, masseCible, mpsForcees)
    setSelection(sel)
  }
